...
#  Equivalent of windows/history.ps1 for bash.
# =============================================================================
...
gReset() {
    ...
    if (( h )); then
        invoke_git reset --hard "${args[0]}"
        ok=$?
        msg="Hard reset to: ${args[0]}"
    elif (( s )); then
        invoke_git reset --soft "${args[0]}"
        ok=$?
        msg="Soft reset to: ${args[0]}"
    elif [[ ${#args[@]} -ge 2 ]]; then
        invoke_git reset "${args[0]}" -- "${args[1]}"
        ok=$?
        msg="File reset: ${args[1]} -> ${args[0]}"
    elif [[ ${#args[@]} -eq 1 ]]; then
        invoke_git reset HEAD -- "${args[0]}"
        ok=$?
        msg="Unstaged: ${args[0]}"
    else
